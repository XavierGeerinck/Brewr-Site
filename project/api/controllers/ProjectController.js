...
module.exports = {
  ...
  show: function(req, res) {
    ...
    User.isMemberOf(user.id, organisation, function(isMember){
      if(!isMember){ return res.badRequest({"success": false, "message": "ORGANISATION_NON_MEMBER"}) }

      Project.belongsTo(project, organisation, function(belongs){
        if(!belongs) { return res.badRequest({"success": false, "message": "PROJECT_NO_BELONG"}) }
   ...
   /* User.isMemberOf(user.id, organisation, function (isMember) {
      ...
      if (!isMember) {
        return res.badRequest({"success": false, "message": "ORGANISATION_NON_MEMBER"})
      }

      // verify that project belongs to organisation
      Project.belongsTo(project, organisation, function(belongsTo, project){

        if(!belongsTo) {
          return req.badRequest({"success": false, "message": "PROJECT_NO_BELONG"});
        }
